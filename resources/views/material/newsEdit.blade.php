                ...
                <div class="ibox">
                    <div class="ibox-content float-e-margins news_box" style="padding: 0px;">
                        ...
                        @foreach($news_item_arr as $key=>$vo)
                            @if($loop->index==0)
                            <dt class="border_show" news_json="{{json_encode($vo,JSON_UNESCAPED_UNICODE)}}">
                                <div class="pic" style="background-image:url('{{$vo['thumb_url']}}');background-size:100%,100%;"></div>
                                <a href="javascript:;" class="tit">标题</a>
                            </dt>
                            @else
                                <dd news_json="{{json_encode($vo,JSON_UNESCAPED_UNICODE)}}">
                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-content controll_box">
                        ...
                        <div class="form-group">
                            ...
                            <div class="row" id="crop_box" style="display: none;">
                                <div class="col-md-6">
...
//                });
            ...
            $('#save').click(function () {
                ...
                $.ajax({
                    headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
                    url: "{{ route('editNewsPost')}}",
                    data: {"news_str":json_str,'media_id':"{{$media_id}}"},
                    type: "post",
                    success:function(res){
                        swal.hideLoading();
                        if(res.code==200){
                            swal('提示','图文素材更新成功', 'success').then(function () {
...
//                    if($('#cover_img').width()>=150 && $('#cover_img').height()>=150){
                    if(_src.indexOf('/ueditor/php/upload/image/')!=-1){
                        img_html += '<li>';
